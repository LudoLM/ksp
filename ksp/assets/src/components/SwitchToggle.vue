...
const switcherToggle = ref(route.path.startsWith('/admin'));
const router = useRouter();

const handleSwitcher = () => {
    switcherToggle.value = !switcherToggle.value;

    if (!switcherToggle.value) {
    ...
    <label for="toggle3" class="flex cursor-pointer select-none items-center">
      <div class="relative">
        <input
          ...
          id="toggle3"
          class="sr-only"
          @click="handleSwitcher"
        />
        <div class="block h-8 w-14 rounded-full bg-indigo-200"></div>
        <div
